...
*/
    ...
    if (net_on) {
        ...
        if (bufferFile) {
            ...
            // Chyba p¯i otevÌr·nÌ souboru bufferu
...
//************************************************************************
// Nov· funkce pro odesl·nÌ CEL…HO bufferu ze souboru aptbuffer.txt
// Zpracuje cel˝ soubor v jednÈ operaci (s yield() uvnit¯)
//************************************************************************
void send_entire_file_buffer() {
    // Zkontrolujeme, zda jsme online a p¯ipojeni
    if (!net_on || !client.connected()) {
    ...
    // SmyËka pro ËtenÌ a odesÌl·nÌ ¯·dk˘
    while (bufferFile.available()) {

        // UvolnÌme procesor pro jinÈ ˙lohy a watchdog
        yield(); // Nebo vTaskDelay(1); pokud m·te RTOS ˙lohy s niûöÌ prioritou
        // Pokud pouûÌv·te watchdog timer a tato funkce trv· dlouho:
        // esp_task_wdt_reset(); 

        String lineToSend = bufferFile.readStringUntil('\n');

        // P¯eskoËÌme potenci·lnÌ pr·zdnÈ ¯·dky
        if (lineToSend.length() == 0 && bufferFile.available()) {
            continue;
        }
        // Pokud je to poslednÌ ¯·dek bez '\n', readStringUntil ho vr·tÌ.
        // Pokud poslednÌ ¯·dek m· '\n', readStringUntil ho odstranÌ.
        // Pro konzistenci p¯id·me '\n' vûdy. Server musÌ b˝t schopen zpracovat i p¯Ìpadn˝ pr·zdn˝ ¯·dek na konci.
        lineToSend += "\n";
        ...
        // Odesl·nÌ dat
        if (client.print(lineToSend)) {
            // client.flush(); // flush m˘ûe b˝t blokujÌcÌ, radÏji poËk·me na odpovÏÔ
            ...
            while (millis() < confirm_timeout) {
                if (client.available()) {
                    ...
                    // Zde upravte logiku kontroly odpovÏdi podle vaöeho serveru!
                    if (response.length() > 0 && response.startsWith("~")) {
                        send_success = true;
                        last_ping = SEC_TIMER;
                        break; // PotvrzenÌ p¯ijato
                    }
                }
                delay(10); // Kr·tk· pauza
            }
            ...
            Serial.println(" - Client print failed.");
            setNetOn(0, 21); // OznaËÌme jako offline
            all_sent_successfully = false; // OznaËÌme chybu
            // ZapÌöeme neodeslan˝ ¯·dek do temp souboru
            tempFile.print(lineToSend);
            break; // UkonËÌme smyËku odesÌl·nÌ
        }

        // Pokud odesl·nÌ nebo potvrzenÌ selhalo
        if (!send_success) {
            all_sent_successfully = false; // OznaËÌme chybu
            // ZapÌöeme neodeslan˝ ¯·dek do temp souboru
            tempFile.print(lineToSend);
            // P¯eruöÌme dalöÌ odesÌl·nÌ v tomto cyklu, protoûe je problÈm
            break;
        }
        // Pokud bylo odesl·nÌ ˙spÏönÈ, ¯·dek se *nezapisuje* do temp souboru

        // Kontrola spojenÌ uvnit¯ smyËky
        if (!net_on || !client.connected()) {
            Serial.println("Connection lost during buffer send.");
            all_sent_successfully = false;
            // ZapÌöeme aktu·lnÌ ¯·dek (kter˝ uû se neodeöle) do temp, pokud jeötÏ nebyl zaps·n
            if (!send_success) { /* Jiû zaps·no v˝öe */ }
            else { tempFile.print(lineToSend); } // Zapsat, pokud by se norm·lnÏ nezapsal
            break; // UkonËit smyËku
        }

    } // Konec while (bufferFile.available())

    // Po ukonËenÌ smyËky (aù uû ˙spÏönÏ nebo chybou):
    // ZkopÌrujeme zbytek p˘vodnÌho souboru do doËasnÈho (pokud smyËka skonËila chybou uprost¯ed)
    ...
    // Fin·lnÌ operace se soubory
    if (all_sent_successfully) {
        // Vöe ˙spÏönÏ odesl·no, doËasn˝ soubor je pr·zdn˝
        Serial.println("Entire buffer file sent successfully. Removing buffer file.");
        if (!LittleFS.remove(bufferFilePath)) {
            Serial.println("Error: Failed to remove original buffer file!");
        }
        // Smaûeme i pr·zdn˝ doËasn˝ soubor
        LittleFS.remove(tempFileName);
    }
    else {
        // Nastala chyba, nebylo vöe odesl·no. Neodeslan· data jsou v tempFile.
        Serial.println("Errors occurred during buffer send. Replacing original with temp file (contains unsent lines).");
        if (LittleFS.remove(bufferFilePath)) { // Smazat p˘vodnÌ
            if (LittleFS.rename(tempFileName, bufferFilePath)) { // P¯ejmenovat doËasn˝ na p˘vodnÌ
                Serial.println("Buffer file updated with remaining data.");
            }
            else {
                Serial.println("Error: Failed to rename temp file!");
                // Ponech·me temp soubor pro p¯Ìpadnou manu·lnÌ kontrolu
            }
        }
        else {
            Serial.println("Error: Failed to remove original buffer file during update!");
            // Ponech·me temp soubor
        }
    }

    Serial.println(">>> File buffer send attempt finished. <<<");
    // Vymaz·nÌ LCD kr·tce po dokonËenÌ
    timeout1 = SEC_TIMER + 1; // »asovaË pro vyËiötÏnÌ LCD ve funkci loop
}