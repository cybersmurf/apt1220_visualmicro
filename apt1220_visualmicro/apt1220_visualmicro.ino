...
// Pole, která drí obsah pro kadı øádek displeje
char g_lcd_line0[21] = { 0 };
char g_lcd_line1[21] = { 0 };
char g_lcd_line2[21] = { 0 };
char g_lcd_line3[21] = { 0 };

// Pøíznak, kterı nám øíká, e se obsah zmìnil a je potøeba pøekreslit displej
bool g_display_dirty = false;

// Èasovaè, kterı nám pomùe detekovat konec dávky zpráv
...
void updateDisplayNow() {
    // Vezmeme si klíè k I2C sbìrnici, abychom mìli jistotu, e nám do toho nic neskoèí
    if (xSemaphoreTake(i2cMutex, portMAX_DELAY) == pdTRUE) {

        // Pouijeme "unsafe" verzi, protoe u dríme zámek (mutex)
    ...
    // A oznaèíme si, e displej je teï "èistı" a není potøeba ho pøekreslovat.
...
void reader_input(const char* display_str, char* data_to_fill) {
    ...
    while (true) {
        if (SerialC.available() > 0) {
            ...
            //else if (strcmp(buffer2, "OK") == 0) {
				...
				saveNewConfigData = true; // Nastavíme flag pro uloení novıch dat
                break;
            }
    ...
    // A po opuštìní smyèky vrátíme klíè
    xSemaphoreGive(i2cMutex);
...
void loop() {
    ...
    if (net_on) {
        TCP(); // Zpracování pøíchozích TCP dat (od serveru) - Mìlo by bıt také blokováno? Záleí na logice.
        ...
        if (!isSendingFileBuffer) {
            ...
            if (fileHasData) {
                ...
                send_entire_file_buffer();  // Pokusí se odeslat celı buffer
        ...
        // Musíme poèkat, a send_entire_file_buffer() dobìhne a flag vynuluje.
    ...
    // Periodická kontrola, napøíklad kadou minutu (pro demonstraci)
    ...
    // Zpracování dat ze sériovıch portù POUZE pokud neodesíláme souborovı buffer
    if (!isSendingFileBuffer) {
        ...
        */
        ...
        // Pokud odesíláme soubor, data ze sériovıch portù nezpracováváme,
    ...
    // Automatickı reset kadıch 24 hodin
    /*
...
void get_time(char* buff, size_t buff_size) {
    ...
    // Vytvoøení formátovaného øetìzce s datem a èasem
    snprintf(buff, buff_size, "%04d-%02d-%02d %02d:%02d:%02d",
        rtc2.getYear(),          // Rok (plnı, napø. 2024)
...
void TCP() {
    ...
    if (!is_connected && net_on == 1) {
        connectToServer_safe(); // Pouijeme náš bezpeènı wrapper, kterı u máme
    }
    ...
    while (client.available() > 0) {
        ...
        case 2: {
            ...
            if (items_parsed == 6) {
                ...
                // Teï správnì nastavíme strukturu tm, kterou pouívá mktime
                ...
                rtc2.stopClock();
                // Pro rtc2.setDate/Time pouijeme pùvodní, plné hodnoty
        ...
        case 0xE: {
            // 1. Pøipravíme si buffer pro celı øádek (20 znakù + koncovı nulovı znak)
            char line_buffer[21];
            ...
            // 3. Sestavíme celı øádek pomocí funkce snprintf.
            // Pouijeme trik s hvìzdièkou v %s, kterı nám umoní dynamicky nastavit
            ...
            // 4. Teï, kdy máme vše pøipraveno, zamkneme I2C a pošleme to na displej
            if (xSemaphoreTake(i2cMutex, portMAX_DELAY) == pdTRUE) {
                lcd2.setCursor(0, 0); lcd2.printf("*------------------*");

                // Pošleme náš hotovı, sestavenı øádek najednou
                lcd2.setCursor(0, 1);
...
void send_entire_file_buffer() {
    ...
    while (bufferFile.available()) {
        ...
        if (client.print(lineToSend)) {
            ...
            while (millis() < confirm_timeout) {
                if (client.available()) {
                    ...
                    // Protoe server mùe poslat více odpovìdí pøed OK
                    while (client.available()) {
                        ...
                        if (response.equals("OK")) {
                            ...
                            last_ping = SEC_TIMER;
                            // U neèekáme dál na této lince, máme potvrzení
        ...
        // 6. Zpracování vısledku pro danı øádek
        if (send_attempt_failed || confirmation_failed) {
            ...
            // Zapsat neúspìšnı øádek do temp souboru
    ...
    // ... (Zbytek funkce - zavøení souborù a finální operace remove/rename - zùstává stejnı jako v minulé odpovìdi) ...
    // 8. Zavøení souborù
    bufferFile.close();
